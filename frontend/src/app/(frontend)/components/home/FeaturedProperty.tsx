...
import { frontendPropertyService, Property } from "@/app/(frontend)/services/propertyService";
...
export function FeaturedProperty() {
  ...
  const nextImage = () => {
    const images = getPropertyImages(currentProperty);
  ...
  const getPropertyImages = (property: Property): string[] => {
    ...
    if (property.featured_image && !imageErrors[`featured-${property.id}`]) {
      images.push(`${baseUrl}/storage/${property.featured_image}`);
    }
    ...
    if (property.images && property.images.length > 0) {
      property.images.forEach((imagePath, index) => {
        ...
        if (!imageErrors[imageKey]) {
          images.push(`${baseUrl}/storage/${imagePath}`);
        }
      });
    }
      ...
      <section className={`py-20 transition-all duration-500 ${
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="animate-pulse">
            {/* Header Skeleton */}
            <div className={`h-8 w-64 mb-4 rounded ${isDarkMode ? "bg-gray-700" : "bg-gray-200"}`} />
            <div className={`h-4 w-96 mb-8 rounded ${isDarkMode ? "bg-gray-700" : "bg-gray-200"}`} />
            
            {/* Content Skeleton */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              <div className={`h-96 rounded-2xl ${isDarkMode ? "bg-gray-700" : "bg-gray-200"}`} />
  ...
  const propertyImages = getPropertyImages(currentProperty);
    ...
    <section className={`py-20 transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="relative">
          ...
          {/* Header with Property Navigation */}
          <div className="flex justify-between items-end mb-12">
            <div>
              <div className={`inline-flex items-center gap-3 rounded-full px-4 py-2 mb-4 ${
                isDarkMode 
            ...
            {/* Property Counter with Navigation */}
              ...
              <div className="flex items-center gap-4 flex-wrap">
                ...
                <div className={`px-3 py-1 rounded-full ${
                  ...
                  <span className={`text-sm font-medium ${
                    ...
                    {currentProperty.status === 'published' ? 'AVAILABLE' : currentProperty.status.toUpperCase()}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  { 
                    ...
                    label: "Size", 
                    value: currentProperty.size ? `${currentProperty.size.toLocaleString()} sqft` : "N/A" 
                  },
                ].map((feat, i) => (
                ...
                {propertyImages.length > 0 ? (
                  <>
                    <div
                      ...
                      onError={() => handleImageError(`featured-${currentProperty.id}-${currentImageIndex}`)}
                    >
                  ...
                  {propertyImages.slice(0, 3).map((image, index) => (
                    ...
                    >
                      <div
                        ...
                        onError={() => handleImageError(`${currentProperty.id}-thumb-${index}`)}
                      />
                    </button>
                  ))}
                  {propertyImages.length > 3 && (
                    ...
                    >
                      <div
                        ...
                        onError={() => handleImageError(`${currentProperty.id}-thumb-3`)}
                      />
                      <div className="absolute inset-0 bg-black/50 flex items-center justify-center group-hover:bg-black/60 transition-colors duration-300">
                        <span className="text-white text-sm font-semibold">+{propertyImages.length - 3}</span>
                      </div>
                    </button>
                  )}
                </div>
